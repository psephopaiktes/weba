...
```
`matches` が対象となるWEBページのURLパターン、`css`,`js`がそのページに読み込ませるCSS,JSファイルの指定です。`*`はワイルドカードです。上記の例であれば、`https://www.google.co.jp/`から始まるURLすべてにマッチングします。

CSS,JSファイルを対象ページごとにフォルダ分けして作成しておきましょう。まだ中身は空白です。

![フォルダ構造](/images/post/201808_chrome-stylize/directory.png)
